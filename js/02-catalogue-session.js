...
function imprimerResultatLPP() {
  ...
  const contenuBilanPrevoyance = `<html><head><title>${titreBilanPrevoyance}</title><style>
    ...
    <script>
      (function(){var t=${JSON.stringify(titreBilanPrevoyance)};document.title=t;var e=document.querySelector('title');if(e)new MutationObserver(function(){if(document.title!==t)document.title=t;}).observe(e,{childList:true,characterData:true,subtree:true});})();
      function telechargerPdfAuto() {
        ...
        var nom = (${JSON.stringify(titreBilanPrevoyance)} || 'Document').replace(/[<>:"/\\\\|?*\\x00-\\x1F]/g, '').trim() + '.pdf';
...
const CATALOGUE_LCA_PAR_COMPAGNIE = {
  helsana: [
    ...
    'Helsana DENTAplus BRONZE (dentaire)',
    'Helsana DENTAplus ARGENT (dentaire)',
    'Helsana DENTAplus OR (dentaire)',
    'Helsana DENTAplus COMBI (dentaire)',
    'Helsana HOSPITAL ECO (hospitalisation)',
    'Helsana HOSPITAL PLUS (hospitalisation)',
    'Helsana HOSPITAL COMFORT (hospitalisation)',
    'Helsana HOSPITAL FLEX (hospitalisation)',
    'Helsana HOSPITAL EXTRA (hospitalisation)',
  ],
  'groupe mutuel': [
