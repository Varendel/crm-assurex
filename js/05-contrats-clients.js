...
function ouvrirOngletDocumentsClient(clientId) {
  ...
  const boutons = [
    ...
    typeof ouvrirDossierFinancement === 'function' ? boutonOngletAction('modal-onglet-documents', `ouvrirDossierFinancement('${clientId}')`, '🏦 Préparer un dossier de financement') : '',
...
function ouvrirOngletAdminClient(clientId) {
  const boutons = [
    ...
    typeof mcEcrireAuClient === 'function' ? boutonOngletAction('modal-onglet-admin', `mcEcrireAuClient('${clientId}')`, '✉️ Écrire au client (motif)') : '',
  ].filter(Boolean).join('');
...
function construireHtmlMandat(champs, signatureDataUrl, signatureMandataire, clausesSpeciales) {
  ...
  return `<html><head><meta charset="utf-8"><title>${titreDoc}</title><style>
    ...
    <ol>
      ...
      <li>Le mandataire précise que son indemnisation provient directement des compagnies d'assurance, et par conséquent, ses prestations ne feront l'objet d'aucune facturation d'honoraires, sauf condition particulière préalablement validée entre les parties. Conformément à l'art. 402 CO, si des frais ou avances devaient exceptionnellement être engagés par le mandataire pour l'exécution régulière du présent mandat, le mandant s'engage à les lui rembourser.</li>
