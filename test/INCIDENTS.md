...
| 6 | 19.09.2026 | Espace client → envoi des accès | Le mot de passe part en clair par e-mail et reste dans la boîte du destinataire | Génération du mot de passe côté CRM puis envoi tel quel. Trois conséquences : le secret restait dans deux boîtes mail, **le courtier le connaissait** (il pouvait donc entrer dans l'espace de son client sans que rien ne l'en distingue dans les traces), et un mot de passe imposé finit recopié sur un papier | Corrigé le 25.09.2026 — lien d'activation à usage unique (`acces-client` v8, `js/165`) ; le compte naît avec un secret que personne ne voit |
...
| 18 | 25.09.2026 | Pose du mandat → envoi à la compagnie | **Un courriel vierge est parti à Zurich** : ni objet, ni une ligne de texte — seulement la signature et le mandat en pièce jointe. Jonathan a dû s'en excuser par un second envoi | Le 24.09, la pose du mandat a été rebranchée sur le rédacteur d'e-mail partagé (`js/07`, champs `apercu-email-…`), mais son bouton d'envoi, resté dans `js/05`, lisait encore les champs de l'ancienne fenêtre (`apercu-mandat-…`). Ces identifiants n'existaient plus : `getElementById(...)?.value \|\| ''` a rendu deux chaînes vides, sans rien signaler. Le bouton « M'envoyer un essai », lui, lisait les bons champs — l'essai était parfait, et c'est ce qui a endormi la méfiance : **un essai ne protège que le chemin qu'il emprunte** | Clos (`envoi-vide.test.js`) — les champs lus sont vérifiés, et `envoyerCourriel` refuse désormais tout message sans objet NI corps |
| 19 | 25.09.2026 | Fil de l'affaire | La pose du mandat n'apparaissait nulle part dans l'affaire : seule trace, « ✓ Tâche terminée : Poser mandat ». À qui, quand, avec quoi — uniquement dans Outlook | Aucun des envois de mandat n'écrivait au fil, contrairement aux offres, relances, rendez-vous et comparatifs | Clos (`fil-affaire.test.js`, `js/166`) |
